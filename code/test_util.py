...
def test_all_case(net, image_list, num_classes, patch_size=(112, 112, 8), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None):
    ...
    for image_path in tqdm(image_list):
        ...
        for z in range(prediction.shape[2]):
            prediction[:,:,z] = ndimage.binary_fill_holes(prediction[:,:,z])
        # prediction_filled = ndimage.binary_fill_holes(prediction_filled)
        ...
        if save_result:
            # nib.save(nib.Nifti1Image(prediction.astype(np.float32), np.eye(4)), test_save_path + id + "_prednofill.nii.gz")
